...
ktrk_localish_data <- read.csv("ktrk-localish-raw-data.csv") %>% 
  janitor::clean_names() %>% 
  filter(video != "NONE")


ktrk_localish_locations <- ktrk_localish_data %>%
  geocode(location, method = 'geocodio', lat = lat , long = lon) 

ktrk_localish_locations <- ktrk_localish_locations %>% 
...
ktrk_localish_locations_sf <- st_as_sf(ktrk_localish_locations, coords = c("lon", "lat"), crs = "4326")

st_write(ktrk_localish_locations_sf, "ktrk_localish_locations.geojson", append = FALSE)
